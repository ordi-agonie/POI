# Contribution

## Méthode de travail

Avant de modifier un fichier existant :
- vérifier sa version actuelle ;
- demander le fichier si son état actuel n'est pas disponible.

Lorsqu'un problème concret est identifié :
- le corriger immédiatement s'il est local et sans risque ;
- sinon créer explicitement une tâche dans `Plan.md`.

Lorsqu'une décision modifie le modèle, l'architecture ou le comportement :
- mettre à jour la documentation concernée avant le commit.

Avant un commit :
- tester le comportement concerné ;
- vérifier que la documentation est cohérente avec le code.

Lorsqu'une modification de code est proposée :
- expliquer les nouvelles fonctions introduites ;
- expliquer les nouvelles syntaxes ou constructions de programmation utilisées lorsqu'elles n'ont pas encore été vues ;
- expliquer les nouveaux concepts de programmation nécessaires à la compréhension du code ;
- indiquer le comportement attendu et le test à effectuer avant de poursuivre.

Lorsqu'un fichier Markdown complet est fourni pour être copié-collé :
- fournir le fichier complet dans un bloc de code ;
- utiliser un délimiteur extérieur suffisamment long pour ne pas interrompre les éventuels blocs de code présents dans le fichier ;
- conserver le contenu Markdown interne tel quel.
  
## Documentation

- `Architecture.md` décrit comment le système est construit.
- `Specification.md` décrit ce que l'application doit faire.
- `Decisions.md` explique pourquoi un choix a été retenu lorsqu'il existe plusieurs solutions possibles.
- `Plan.md` décrit les tâches et leur état d'avancement.

Lorsqu'une information pourrait appartenir à plusieurs documents, privilégier le document correspondant à sa fonction plutôt que de la dupliquer.